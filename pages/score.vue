<template>
    ...
    <tbody class="w-full" v-for="(player, index) in players" :key="player.name" >
        <tr >
            ...
            <td class="border border-gray-400 px-4 py-2">{{ player.name }} <DxButton text="edit"/></td>
            <td class="border border-gray-400 px-4 py-2">{{ player.matches }} 
                <DxButton @click="increaseMatches(index)" text="+"/>
                <DxButton @click="decreaseMatches(index)" text="-"/>
            </td>
            <td class="border border-gray-400 px-4 py-2">{{ player.longestR }} 
                <DxButton text="+"/>
                <DxButton text="-"/>
            </td>
            <td class="border border-gray-400 px-4 py-2">{{ player.largestA }} 
                <DxButton text="+"/>
                <DxButton text="-"/>
            </td>
            
            <td class="border border-gray-400 px-4 py-2">{{wins*3 }}
            </td>
             <td class="border border-gray-400 px-4 py-2">{{wins*3 }}
            </td>
        </tr>
